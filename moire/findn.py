"""Reference-layer N-layer commensuration search for CELLSTINE."""
...
def format_results_table(candidates: Sequence[NLayerCandidate], limit: int | None = None) -> str:
    shown = list(candidates if limit is None or limit < 0 else candidates[: max(limit, 0)])
    if not shown:
        return "No N-layer candidates found."

    header = " idx  strain_max  strain_mean   atoms  ratio_bottom  upper angles (deg)"
    separator = "-" * len(header)
    lines = [header, separator]
    for index, candidate in enumerate(shown, start=1):
        angle_summary = ", ".join(f"L{layer.layer_index + 1}={layer.angle_deg:.3f}" for layer in candidate.upper_layers)
        lines.append(
            "{idx:4d}  {strain_max:10.6f}  {strain_mean:11.6f}  {atoms:7d}  {ratio_bottom:12d}  {angles}".format(
                idx=index,
                strain_max=candidate.strain_max,
                strain_mean=candidate.strain_mean,
