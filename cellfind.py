...
def build_supercells(pairs, lin_tol, rot1, lat2, cnt1, cnt2, angle=None):
    """From matched vector pairs build candidate supercells.

    Returns list of tuples containing
    (strain_avg, strain_layer1, strain_layer2, om1, om2, atoms,
    ...
    for p in range(k):
        for q in range(p, k):
            ...
            # layer‑1 strain required to match layer‑2 lengths
            s1 = math.sqrt(((n_g1/n_v1 - 1)**2 + (n_g2/n_v2 - 1)**2)/2) if n_v1>0 and n_v2>0 else 0.0
            # layer‑2 strain required to match layer‑1 lengths
...
def process_angle(args):
    ...
    return build_supercells(pairs, lin_tol, rot1, lat2[:2], cnt1, cnt2, angle)
...
def main():
    ...
    parser.add_argument('--strain_tol', type=float, default=None,
                        help='reject candidates with strain above this value')
    parser.add_argument('--strain_layer', choices=['avg','1','2'], default='avg',
    ...
    for rec in flat:
        ...
        if args.max_atoms is not None and atoms > args.max_atoms:
            continue
        if args.min_atoms is not None and atoms < args.min_atoms:
            continue
        if args.strain_tol is not None:
            if args.strain_layer == 'avg' and strain_avg > args.strain_tol:
                continue
            if args.strain_layer == '1' and s1 > args.strain_tol:
                continue
            if args.strain_layer == '2' and s2 > args.strain_tol:
                continue
        filtered.append(rec)

    # Unique by average strain and cell ratios, keep smallest atom count
    uniq = {}
    for rec in filtered:
        strain_avg, s1, s2, om1, om2, atoms, *rest = rec
        key = (round(strain_avg / args.lin_tol), om1, om2)
        if key not in uniq or atoms < uniq[key][5]:
            uniq[key] = rec
    results = sorted(uniq.values(), key=lambda x: x[0])

    # Output to file with enhanced header
    with open(args.output, 'w') as f:
        ...
        for idx, rec in enumerate(results, 1):
            ...
            f.write(
                f"|{idx:4d} | {ang:10.4f} | {strain_avg:10.6f} | {s1:8.6f} | {s2:8.6f} | "
    ...
    for idx, rec in enumerate(results, 1):
        ...
        j11, j12 = c2p; j21, j22 = c2q
        print(
            f"|{idx:4d} | {ang:10.4f} | {strain_avg:10.6f} | {s1:8.6f} | {s2:8.6f} | {atoms:5d} | "
            f"{om1:3d}/{om2:<3d} | {i11:4d} {i12:4d} | {i21:4d} {i22:4d} | {j11:4d} {j12:4d} | "
