...
def _orbits_from_permutations(permutations: np.ndarray, count: int) -> np.ndarray:
    ...
    are proved in ``RequestProject/SiteOrbits.lean``
...
def translation_group(
    ...
    *,
    ...
    ``RequestProject/CenteringLattice.lean``), its order divides the number of
...
def _canonical_planar_gauge(basis: np.ndarray, cell: np.ndarray) -> np.ndarray:
    ...
    ``RequestProject/PlaneGauge.lean``: two reduced bases of one plane lattice
