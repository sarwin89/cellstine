...
def test_moire_plain_parser_group_is_split_from_plain_frontend():
    import inspect

    import cellstine.cli.plain as plain
    from cellstine.cli.plain_moire import add_moire_group

    assert callable(add_moire_group)
    assert "add_moire_group(groups)" in inspect.getsource(plain.build_parser)


def test_moire_plain_parser_group_imports_without_numpy_or_workflows():
    script = textwrap.dedent(
        f"""
        import importlib.util, sys
        from pathlib import Path
        ...
        sys.modules["cellstine"] = module
        spec.loader.exec_module(module)
        import cellstine.cli.plain_moire
        print("numpy" in sys.modules)
        print(",".join(sorted(name for name in sys.modules if name.startswith("cellstine."))))
        """
    )
...
def test_surface_and_interface_plain_parser_groups_are_split_from_plain_frontend():
    import inspect

    import cellstine.cli.plain as plain
    from cellstine.cli.plain_interface import add_interface_group
    from cellstine.cli.plain_surface import add_surface_group

    source = inspect.getsource(plain.build_parser)
    assert callable(add_surface_group)
    assert callable(add_interface_group)
    assert "add_surface_group(groups)" in source
    assert "add_interface_group(groups)" in source


@pytest.mark.parametrize("module_name", ["plain_surface", "plain_interface"])
def test_split_plain_parser_groups_import_without_numpy_or_workflows(module_name):
    script = textwrap.dedent(
        f"""
        import importlib.util, sys
        from pathlib import Path
        ...
        sys.modules["cellstine"] = module
        spec.loader.exec_module(module)
        __import__("cellstine.cli.{module_name}")
        print("numpy" in sys.modules)
        print(",".join(sorted(name for name in sys.modules if name.startswith("cellstine."))))
        """
    )
...
def test_adsorbate_plain_parser_group_is_split_from_plain_frontend():
    import inspect

    import cellstine.cli.plain as plain
    from cellstine.cli.plain_adsorbate import add_adsorbate_group

    source = inspect.getsource(plain.build_parser)
    assert callable(add_adsorbate_group)
    assert "add_adsorbate_group(groups)" in source


def test_adsorbate_plain_parser_group_imports_without_numpy_or_workflows():
    script = textwrap.dedent(
        f"""
        import importlib.util, sys
        from pathlib import Path
        ...
        sys.modules["cellstine"] = module
        spec.loader.exec_module(module)
        import cellstine.cli.plain_adsorbate
        print("numpy" in sys.modules)
        print(",".join(sorted(name for name in sys.modules if name.startswith("cellstine."))))
        """
    )
...
def test_view_plain_parser_group_is_split_from_plain_frontend():
    import inspect

    import cellstine.cli.plain as plain
    from cellstine.cli.plain_view import add_view_group

    source = inspect.getsource(plain.build_parser)
    assert callable(add_view_group)
    assert "add_view_group(groups)" in source


def test_view_plain_parser_group_imports_without_numpy_or_workflows():
    script = textwrap.dedent(
        f"""
        import importlib.util, sys
        from pathlib import Path
        ...
        sys.modules["cellstine"] = module
        spec.loader.exec_module(module)
        import cellstine.cli.plain_view
        print("numpy" in sys.modules)
        print(",".join(sorted(name for name in sys.modules if name.startswith("cellstine."))))
        """
    )
...
def test_symmetry_plain_parser_group_is_split_from_plain_frontend():
    import inspect

    import cellstine.cli.plain as plain
    from cellstine.cli.plain_symmetry import add_symmetry_group

    source = inspect.getsource(plain.build_parser)
    assert callable(add_symmetry_group)
    assert "add_symmetry_group(groups)" in source


def test_symmetry_plain_parser_group_imports_without_numpy_or_workflows():
    script = textwrap.dedent(
        f"""
        import importlib.util, sys
        from pathlib import Path
        ...
        sys.modules["cellstine"] = module
        spec.loader.exec_module(module)
        import cellstine.cli.plain_symmetry
        print("numpy" in sys.modules)
        print(",".join(sorted(name for name in sys.modules if name.startswith("cellstine."))))
        """
    )
